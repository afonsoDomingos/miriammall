...
export async function GET() {
  try {
    ...
    const buildings = await db.default.connection.db.collection('buildings').find({}).sort({ order: 1 }).toArray();
...
export async function POST(req: Request) {
  try {
    ...
    const result = await db.default.connection.db.collection('buildings').insertOne(newBuilding);
